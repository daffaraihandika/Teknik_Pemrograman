...
public class Restaurant {
    ...
    public void takeOrder(String nama_makanan, int banyak){
        // buat algo searching
        for(int i=0; i<=id; i++){
            if(nama_makanan == getNamaMakanan(i)){
                if(!isOutOfStock(i)){
                    this.setStok(getStok(i) - banyak, i);
                }
            }
        }
    }
    ...
    public void tampilMenuMakanan2(){
        for(int i =0; i<=id;i++){
            if(!isOutOfStock(i)){
                ...
                System.out.println(getNamaMakanan(i) + " stok sudah habis");
            }
        }
    }
